...
public class DownloadManager extends AsyncHttpClient
{
     ...
     * @see [类、类#方法、类#成员]
     */
    public void addHandler(int messageId, String url, DownLoadListening listener)
    {
        RequestParam param = new RequestParam(messageId, url);
        doDownload(param, listener);
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public void addHandler(int messageId, String url, int start, int end, DownLoadListening listener)
    {
        RequestParam param = new RequestParam(messageId, url);
     ...
     * @see [类、类#方法、类#成员]
     */
    public void addHandler(RequestParam param,DownLoadListening listener)
    {
        doDownload(param, listener);
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    private void doDownload(RequestParam param, DownLoadListening listener)
    {
        doDownload(param, null,listener);
    }
    ...
    private void doDownload(RequestParam param, String fileName, DownLoadListening listener)
    {
        ...
        else
        {
            ...
            else if (!StringUtils.isEmpty(param.getUrl()))
            {
                ...
                {
                    fileName = setFileName(StringUtils.getUrlFileName(param.getUrl()));
                }
    ...
    {
        int number = 0;// 编号,用于如果要下载的文件已存在的话用来编号使用,跟在文件名后
        ...
        while (file.exists())
        {
            number++;
            fileName =
                StringUtils.getPrefixByFileName(fileName) + "(" + number + ")"
                    + StringUtils.getFileTypeByFileName(fileName);
            file = null;
