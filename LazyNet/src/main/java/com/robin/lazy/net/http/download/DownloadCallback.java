...
public class DownloadCallback extends Handler
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case PROGRESS_MESSAGE:
                if (downloadListening != null) {
                    downloadListening.onLoading(messageId, (Long) response[0],
                            (Long) response[1]);
                }
                break;
            case SPEED_MESSAGE:
                if (downloadListening != null) {
                    downloadListening.onSpeed(messageId, (Long) response[0]);
                }
                break;
            ...
            case FAILURE_MESSAGE:
                if (downloadListening != null) {
                    downloadListening
                            .onFailure(messageId, msg.arg2,(String) response[0]);
                }
                break;
            case CANCEL_MESSAGE:
                if (downloadListening != null) {
    ...
    public void downloadStart(int messageId) {
        obtainMessage(START_MESSAGE, messageId,0).sendToTarget();
    ...
    public void downloadProgress(int messageId, long bytesWritten,
        ...
        if (curTime - lastTime >= 100) {
            obtainMessage(PROGRESS_MESSAGE, messageId, 0,
                    new Object[]{bytesTotal, bytesWritten})
                    .sendToTarget();
            lastTime = curTime;
        }
        if (intent != null && context != null) {
    ...
    public void downloadSpeed(int messageId, long speed) {
        obtainMessage(SPEED_MESSAGE, messageId, 0, new Object[]{speed})
                .sendToTarget();
        if (intent != null && context != null) {
            intent.putExtra(DownloadReceiver.KEY_STATUS,
    ...
    public void downloadSuccess(int messageId) {
        obtainMessage(SUCCESS_MESSAGE, messageId, 0).sendToTarget();
        if (intent != null && context != null) {
            intent.putExtra(DownloadReceiver.KEY_STATUS,
    ...
    public void downloadFail(int messageId, int statusCode) {
        if (statusCode == HttpError.USER_CANCEL) {
            obtainMessage(CANCEL_MESSAGE, messageId, 0)
                    .sendToTarget();
        } else {
            obtainMessage(FAILURE_MESSAGE, messageId, statusCode,
                    new Object[]{HttpError.getMessageByStatusCode(statusCode)})
                    .sendToTarget();
        }
        if (intent != null && context != null) {
            intent.putExtra(DownloadReceiver.KEY_STATUS,
